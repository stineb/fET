...
df_lin_conservative <- df_lin %>% 
  mutate(str = "conservative", cwd = cumsum(transp))

df_lin_exploit <- tibble(time = 1:ntsteps, soilm = s_t, transp = t_t) %>% 
  mutate(str = "exploit", cwd = cumsum(transp))

df_lin <- bind_rows(df_lin_conservative, df_lin_exploit)

df_lin %>% 
  ggplot(aes(x = time, y = soilm, color = str)) + 
  geom_line()

df_lin %>% 
  tidyr::drop_na() %>% 
  ggplot(aes(time, transp, color = str)) + 
  geom_line()

df_lin %>%
  tidyr::drop_na() %>% 
  ggplot(aes(x = cwd, y = transp, color = str)) +
  labs(x=expression(integral(ET))) +
  geom_line()
```
